...
exports.run = async (client, message, args) => {
    ...
    if (!args[0]) return message.channel.send('**Lütfen Bir Emoji Log Kanalı Belirtin!**');
    ...
    if(args[0] !== "sıfırla"){
        ...
        if(log.type === "voice") return message.channel.send('Lütfen Bir Yazı Kanalı Belirtin!');
            ...
            db.set(`emojilog_${message.guild.id}`, log.id)

            const embed = new MessageEmbed()

                .setTitle('Başarılı')
                .setDescription(`Emoji Log Kanalı Başarıyla ${log} Olarak Ayarlandı!`)
                .setFooter('Sıfırlamak İçin o!emoji-log sıfırla')
                .setColor('GREEN')
    ...
    if(args[0] === "sıfırla"){
         db.delete(`emojilog_${message.guild.id}`)

         return message.channel.send(
             new MessageEmbed()
             .setTitle('Başarılı')
             .setDescription(`**Emoji Log Kanalı Başarıyla Sıfırlandı!**`)
             .setColor('GREEN')
             )
    }
}
