...
exports.run = async (client, message, args) => {
    ...
    if (!args[0]) return message.channel.send('**Lütfen Bir Mesaj Log Kanalı Belirtin!**');
    ...
    if (args[0] !== "sıfırla") {
        ...
        if (log.type === "voice") return message.channel.send('Lütfen Bir Yazı Kanalı Belirtin!');
            ...
            db.set(`mesajlog_${message.guild.id}`, log.id)

            const embed = new MessageEmbed()

                .setTitle('Başarılı')
                .setDescription(`Mesaj Log Kanalı Başarıyla ${log} Olarak Ayarlandı!`)
                .setFooter('Sıfırlamak İçin o!mesaj-log sıfırla')
                .setColor('GREEN')
